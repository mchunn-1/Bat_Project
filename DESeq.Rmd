...
dat <- read.csv("intAll.csv", header=T, row.names = 1)
...
```{r}
dds <- DESeqDataSetFromMatrix(dat, info, ~diet)
```
...
###uncomment to export normalized read count
...
###uncomment to export ordered file 
...
#volcano plot.. 
ggplot(deSeqRes, aes(x = log2FoldChange, y = -log10(padj), color = Significant)) + geom_point() 
...
pheatmap(log2(sigCounts + 1), scale = "row", show_rownames = F, treeheight_row = 0, treeheight_col = 50)
```
